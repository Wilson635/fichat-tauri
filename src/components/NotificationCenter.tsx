...
export function NotificationCenter() {
  ...
  const showPermissionBtn = !notifGranted;
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => setShowDndConfig((v) => !v)}
                className="flex items-center gap-1.5 text-xs px-2 py-1 rounded-lg transition-colors"
                style={{
                  backgroundColor: dndEnabled
                    ? "rgba(239,68,68,0.1)"
              ...
              {dndEnabled && (
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDnd(dndEnabled, Number(e.target.value), dndEndHour)
                    }
                    className="w-12 text-center rounded-lg border px-1 py-0.5 text-xs"
                    style={{
                      backgroundColor: "var(--color-surface)",
                      borderColor: "var(--color-border)",
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDnd(dndEnabled, dndStartHour, Number(e.target.value))
                    }
                    className="w-12 text-center rounded-lg border px-1 py-0.5 text-xs"
                    style={{
                      backgroundColor: "var(--color-surface)",
                ...
                <button
                  ...
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.backgroundColor =
                      item.isRead ? "transparent" : "var(--color-active)";
                  }}
                >
                  <div className="relative shrink-0 mt-0.5">
                    ...
                    >
                      {item.isPriority
                        ? "!"
                        : item.conversationName.slice(0, 2).toUpperCase()}
                    </div>
                    {!item.isRead && (
                      <span
                        className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-blue-500 border-2"

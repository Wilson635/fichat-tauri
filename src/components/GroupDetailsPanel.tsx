...
export function GroupDetailsPanel({ conversation, onClose }: Props) {
  ...
  const mockDocs = [
    { name: "Rapport_Q3_2025.pdf", size: "2.4 Mo", type: "pdf", sender: "Alice Martin" },
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col items-center py-6 px-4" style={{ backgroundColor: "var(--color-header-bg)" }}>
          <div
            ...
            style={{ backgroundColor: `hsl(${hue(conversation.name)}, 55%, 45%)` }}
          >
            {conversation.name.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase()}
          </div>
          <h2 className="font-bold text-lg text-center" style={{ color: "var(--color-text-primary)" }}>
            {conversation.name}
          </h2>
        ...
        {tab === "media" && (
          <div className="p-4 space-y-4">
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--color-text-muted)" }}>Photos & Vidéos</h4>
              <div className="grid grid-cols-3 gap-1">
                {mockMedia.map((m) => (
                  <div key={m.id} className="aspect-square rounded-lg flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity" style={{ backgroundColor: m.color }}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-white opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--color-text-muted)" }}>Documents</h4>
                ...
                {mockDocs.map((d) => (
                  <div key={d.name} className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ backgroundColor: "var(--color-surface-secondary)" }}>
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center text-xs font-bold text-white shrink-0"
                      style={{ backgroundColor: d.type === "pdf" ? "#ef4444" : d.type === "xlsx" ? "#16a34a" : "#2563eb" }}>
                      {d.type.toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium truncate" style={{ color: "var(--color-text-primary)" }}>{d.name}</div>
                      <div className="text-xs" style={{ color: "var(--color-text-muted)" }}>{d.size} · {d.sender}</div>
                    </div>
